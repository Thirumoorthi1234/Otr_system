...
// Fetch OJT Stages
$stmt = $pdo->prepare("SELECT s.*, u.full_name as trainer_name FROM training_stages s LEFT JOIN users u ON s.trainer_id = u.id WHERE s.assignment_id = ? AND s.type = 'ojt' ORDER BY s.certified_date ASC");
$stmt->execute([$assignment_id]);
...
<head>
    <meta charset="UTF-8">
    <title>OJT TRAINING - SHOP FLOOR | <?php echo e($assignment['trainee_name']); ?></title>
    <style>
