...
if (!$assignment_id) {
    ...
    <div class="card" style="max-width: 800px; margin: 40px auto;">
        <h3 style="margin-bottom: 20px;">Select Training Assignment</h3>
        <p style="color: var(--text-muted); margin-bottom: 20px;">Please select an active training assignment to manage its practical stages and induction progress.</p>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><span class="badge <?php echo $row['status'] == 'completed' ? 'badge-success' : 'badge-warning'; ?>"><?php echo strtoupper(str_replace('_', ' ', $row['status'])); ?></span></td>
                        <td><a href="training_hub.php?id=<?php echo $row['id']; ?>" class="btn btn-primary" style="padding: 5px 12px; font-size: 0.8rem;">Manage</a></td>
                    </tr>
                    <?php endwhile; ?>
                    <?php if ($stmt->rowCount() == 0): ?>
                        <tr><td colspan="5" style="text-align: center; color: var(--text-muted); padding: 30px;">No training assignments found.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; background: white; padding: 25px; border-radius: 12px; border: 1px solid #edf2f7; box-shadow: 0 4px 6px rgba(0,0,0,0.02);">
        <div>
            ...
            <p style="margin:5px 0 0; color: var(--text-muted); font-size: 0.9rem;">
                Emp ID: <strong><?php echo e($assignment['trainee_eno']); ?></strong> | 
                Dept: <strong><?php echo e($assignment['trainee_dept']); ?></strong> | 
                Module: <strong><?php echo e($assignment['module_name']); ?></strong>
            </p>
        </div>
        <div style="display: flex; gap: 10px;">
            ...
            <a href="<?php echo ($_SESSION['role'] == 'admin') ? 'assignments.php' : '../trainer/trainees.php'; ?>" class="btn" style="background:#eee; color:#555;">Back</a>
        </div>
    </div>
</div>
...
<div class="grid" style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 30px;">
    ...
    <div style="grid-column: span 2;">
        <div class="card">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h3>Practical & Shop Floor Training Stages</h3>
                <button onclick="document.getElementById('stage-form').scrollIntoView()" class="btn btn-primary" style="font-size: 0.8rem; padding: 5px 12px;">+ Add New Stage</button>
            </div>
            ...
            <div class="table-container">
                <table>
                    ...
                    <tbody>
                        ...
                        while ($st = $stmt->fetch()):
                            $type_label = [
                                'sdc' => '<span class="badge badge-primary">Practical (SDC)</span>',
                                'otj' => '<span class="badge badge-success">OTJ (Shop Floor)</span>',
                                'recertification' => '<span class="badge badge-warning">Recertification</span>'
                            ][$st['type']];
                        ?>
                        <tr>
                            ...
                            <td>
                                <div style="display: flex; gap: 15px;">
                                    <a href="training_hub.php?id=<?php echo $assignment_id; ?>&edit_stage=<?php echo $st['id']; ?>#stage-form" style="color: var(--primary-blue);"><i class="fas fa-edit"></i> Edit</a>
                                    <a href="training_hub.php?id=<?php echo $assignment_id; ?>&delete_stage=<?php echo $st['id']; ?>" style="color: var(--danger);" onclick="return confirm('Delete this training record?')"><i class="fas fa-trash"></i></a>
                                </div>
                            </td>
                        </tr>
                        <?php endwhile; ?>
                        <?php if ($stmt->rowCount() == 0): ?>
                            <tr><td colspan="5" style="text-align: center; color: var(--text-muted); padding: 40px;">No stages certified yet.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
            ...
            <div id="stage-form" style="margin-top: 40px; border-top: 1px solid #eee; padding-top: 30px;">
                <h4 style="margin-bottom: 20px;"><?php echo $edit_stage ? 'Update Stage Details' : 'Certify New Training Stage'; ?></h4>
                <form method="POST" style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px;">
                    ...
                    <div class="form-group" style="grid-column: span 2;">
                        <label class="form-label">Stage Name / Course</label>
                    ...
                    <div style="grid-column: span 4; display: flex; gap: 10px; margin-top: 10px;">
                        <button type="submit" name="save_stage" class="btn btn-primary" style="padding: 10px 30px;">
                            <?php echo $edit_stage ? 'Update Record' : 'Save Training Record'; ?>
                        </button>
                        <?php if ($edit_stage): ?>
                            <a href="training_hub.php?id=<?php echo $assignment_id; ?>" class="btn" style="background:#eee; color:#333;">Cancel</a>
                        <?php endif; ?>
                    </div>
                </form>
            </div>
    ...
    <div id="induction" style="grid-column: span 2;">
        <div class="card">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h3>Induction Completion Matrix</h3>
                <a href="induction_topics.php" class="btn" style="font-size: 0.8rem; background: #EDF2F7; color: var(--primary-blue);">Manage Master Topics</a>
            </div>
