...
if (isset($_POST['save_exam'])) {
    ...
    if ($_POST['exam_id']) {
        ...
        $message = __("Exam created!");
    }
    $action = 'list';
}

renderHeader(__('exam_management'));
renderSidebar($_SESSION['role']);
?>

<div class="card">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
        <h3><?php echo $action == 'add' ? __('create_exam') : ($action == 'edit' ? __('edit_exam') : __('online_exams')); ?></h3>
        <?php if ($action == 'list'): ?>
            <a href="exams.php?action=add" class="btn btn-primary"><?php echo __('create_exam'); ?></a>
        <?php else: ?>
            <a href="exams.php" class="btn" style="background: #4A5568; color: white;"><?php echo __('back_to_list'); ?></a>
        <?php endif; ?>
    </div>
    ...
    <?php if ($action == 'list'): ?>
        <div class="table-container">
            <table>
                <thead>
                    <tr>
                        <th style="text-align: left;"><?php echo __('exam_title'); ?></th>
                        <th style="text-align: left;"><?php echo __('training_module'); ?></th>
                        <th style="text-align: center;"><?php echo __('duration'); ?></th>
                        <th style="text-align: center;"><?php echo __('pass_percentage'); ?></th>
                        <th style="text-align: center;"><?php echo __('questions'); ?></th>
                        <th style="text-align: center;"><?php echo __('camera'); ?></th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="text-align: center; color: var(--text-muted);"><?php echo e($e['duration_minutes']); ?> <?php echo __('min'); ?></td>
                        <td style="text-align: center;"><?php echo e($e['passing_score']); ?>%</td>
                        <td style="text-align: center;"><span class="badge badge-warning" style="background: #FFF5F1; color: #F59E0B;"><?php echo $e['q_count']; ?> <?php echo __('Qs'); ?></span></td>
                        <td style="text-align: center;">
                            <span class="badge <?php echo $e['camera_enabled'] ? 'badge-success' : 'badge-danger'; ?>" style="<?php echo $e['camera_enabled'] ? 'background:#E6FFFA; color:#069669;' : 'background:#FFF5F5; color:#E53E3E;'; ?>">
                                ...
                                <?php echo $e['camera_enabled'] ? __('camera_on') : __('camera_off'); ?>
                            </span>
                        </td>
                        <td style="text-align: center;">
                            <a href="questions.php?exam_id=<?php echo $e['id']; ?>" style="color: var(--primary-blue); margin-right: 15px;" title="<?php echo __('manage_questions'); ?>"><i class="fas fa-question-circle"></i></a>
    ...
    <form method="POST" action="exams.php">
        ...
        <div class="form-group">
            <label class="form-label"><?php echo __('exam_title'); ?></label>
            <input type="text" name="title" class="form-control" value="<?php echo e($e['title']); ?>" required>
        </div>
        <div class="form-group">
            <label class="form-label"><?php echo __('training_module'); ?></label>
        ...
        <div class="form-group" style="margin-top: 15px;">
            ...
            <div style="display: flex; align-items: center; gap: 12px; padding: 14px 18px; background: var(--sidebar-hover); border-radius: 14px; border: 1px solid var(--border-color);">
                <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; font-weight: 600; font-size: 0.9rem; font-family: 'Outfit', sans-serif;">
                    ...
                    <?php echo __('enable_camera_proctoring'); ?>
                </label>
            </div>
            <p style="font-size: 0.75rem; color: var(--text-muted); margin-top: 6px;"><i class="fas fa-info-circle"></i> <?php echo __('camera_proctoring_desc'); ?></p>
        </div>
        <button type="submit" name="save_exam" class="btn btn-primary"><?php echo __('save_exam'); ?></button>
    </form>
    <?php endif; ?>
</div>
