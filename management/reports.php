...
<style>
    ...
    /* Print Specific Fixes to Ensure 1 Page */
    .print-wrapper {
        width: 100%;
    ...
    .report-nav-group { flex-wrap: wrap; gap: 15px; }
    .premium-tabs { flex-wrap: wrap; }
    
    /* Responsive Overrides for Reports */
    @media (max-width: 1300px) {
        .report-nav-group { flex-direction: column; align-items: stretch !important; gap: 15px; }
        .premium-tabs { justify-content: center; }
    }
    @media (max-width: 1100px) {
        .chart-grid { grid-template-columns: 1fr; }
        .dashboard-container { padding: 25px; }
        .db-header { flex-direction: column; align-items: flex-start; gap: 20px; text-align: left !important; }
        .db-header div { text-align: left !important; }
    }
    @media (max-width: 768px) {
        .stat-grid { grid-template-columns: 1fr; gap: 15px; }
        .dashboard-container { padding: 15px !important; border-radius: 12px; }
        .db-title { font-size: 1.4rem; }
        .stat-value { font-size: 1.8rem; }
        .emp-section-heading { font-size: 1rem; }
        .employee-data-table { display: block; overflow-x: auto; -webkit-overflow-scrolling: touch; }
        .chart-grid { grid-template-columns: 1fr; gap: 20px; }
        .stat-card, .chart-card { padding: 15px !important; }
    }
...
<div class="card" style="border: none; background: transparent; padding: 0; box-shadow: none;">
    ...
    <div class="print-wrapper" id="report-content">
        ...
        <div class="dashboard-container">
            ...
            <div class="stat-grid">
                <div class="stat-card" style="--border-top: #3B82F6;">
                    <style>.stat-card:nth-child(1)::before { background: #3B82F6; }</style>
                    <div class="stat-label">Total Assessments</div>
                    <div class="stat-value"><?php echo number_format($stats['scores']['total'] ?? 0); ?></div>
                </div>
                <div class="stat-card">
                    <style>.stat-card:nth-child(2)::before { background: #8B5CF6; }</style>
                    <div class="stat-label">Average Global Score</div>
                    <div class="stat-value" style="color: #8B5CF6;"><?php echo number_format(round($stats['scores']['avg'] ?? 0, 1), 1); ?>%</div>
                </div>
                <div class="stat-card">
                    <style>.stat-card:nth-child(3)::before { background: #10B981; }</style>
                    <div class="stat-label">Completed Modules</div>
                    <div class="stat-value" style="color: #10B981;"><?php echo number_format($stats['completion']['completed'] ?? 0); ?></div>
                </div>
                <div class="stat-card">
                    <style>.stat-card:nth-child(4)::before { background: #F59E0B; }</style>
                    <div class="stat-label">In-Progress Modules</div>
                    <div class="stat-value" style="color: #F59E0B;"><?php echo number_format($stats['completion']['in_progress'] ?? 0); ?></div>
                </div>
            </div>
        ...
        <div class="dashboard-container employee-report-card">
            ...
            <div style="display: flex; gap: 30px; margin-bottom: 40px; background: #F8FAFC; padding: 25px; border-radius: 16px; border: 1px solid #E2E8F0;">
                ...
                <div style="flex: 1;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: y-15px; row-gap: 15px;">
            ...
            <table class="employee-data-table">
                <thead>
                    <tr>
                        <th style="text-align: center;">About Syrma</th>
                        <th style="text-align: center;">Safety Test</th>
                        <th style="text-align: center;">PPE Test</th>
                        <th style="text-align: center;">5S Test</th>
                        <th style="text-align: center;">ESD Test</th>
                        <th style="text-align: center;">E-Module Test</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if ($induction): ?>
            ...
            <table class="employee-data-table">
                <thead>
                    <tr>
                        <th style="text-align: left;">Module Assignments</th>
                        <th style="text-align: center; width:160px;">Date Assigned</th>
                        <th style="text-align: center; width:160px;">Status</th>
                        <th style="text-align: center; width:160px;">Date Completed</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($assignments as $a): ?>
                    <tr>
                        <td style="font-weight: 700; color: #334155; text-align: left;"><?php echo e($a['module_name']); ?></td>
                        <td style="text-align: center;"><?php echo date('d M Y', strtotime($a['assigned_date'])); ?></td>
                        <td style="text-align: center; color: <?php echo $a['status'] == 'completed' ? '#10B981' : '#F59E0B'; ?>; font-weight: 800;"><?php echo strtoupper($a['status']); ?></td>
                        <td style="text-align: center;"><?php echo $a['completion_date'] ? date('d M Y', strtotime($a['completion_date'])) : '-'; ?></td>
                    </tr>
            ...
            <table class="employee-data-table">
                <thead>
                    <tr>
                        ...
                        <th style="text-align: center; width:160px;">Date Taken</th>
                        <th style="text-align: center; width:120px;">Score</th>
                        <th style="text-align: center; width:160px;">Accreditation</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($exams as $ex): ?>
                    <tr>
                        <td style="font-weight: 700; color: #334155; text-align: left;"><?php echo e($ex['exam_name']); ?></td>
                        <td style="text-align: center;"><?php echo date('d M Y', strtotime($ex['exam_date'])); ?></td>
                        <td style="font-weight: 800; color: #0F172A; text-align: center;"><?php echo $ex['score']; ?>%</td>
                        <td style="text-align: center;"><span style="background: <?php echo $ex['status'] == 'pass' ? '#DCFCE7' : '#FEE2E2'; ?>; color: <?php echo $ex['status'] == 'pass' ? '#16A34A' : '#DC2626'; ?>; padding: 4px 12px; border-radius: 6px; font-weight: 800; font-size: 0.8rem; text-transform: uppercase;"><?php echo $ex['status']; ?></span></td>
                    </tr>
        ...
        <?php elseif ($report_type == 'completion' || $report_type == 'performance'): ?>
        <!-- ============================================== -->
        <!-- 3. LIST REPORTS (Completion & Performance) -->
        <!-- ============================================== -->
        <div class="dashboard-container">
            <div class="db-header">
                <div>
                    <img src="<?php echo BASE_URL; ?>assets/img/profiles/logo.svg" style="height: 35px; margin-bottom: 10px;" onerror="this.style.display='none';">
                    <div class="db-title"><?php echo $report_type == 'completion' ? 'Training Completion List' : 'Performance Log'; ?></div>
            ...
            <table class="employee-data-table">
                ...
                <tbody>
                    ...
                    ?>
                    <tr>
                        <td style="font-weight: 700; color: #0F172A; text-align: left;"><?php echo e($row['trainee_name']); ?></td>
                        <td style="text-align: left;"><?php echo e($row['module_name']); ?></td>
                        <td style="text-align: left;"><?php echo e($row['trainer_name']); ?></td>
                        <td style="text-align: center;"><span style="background: <?php echo $row['status'] == 'completed' ? '#DCFCE7' : '#FEF3C7'; ?>; color: <?php echo $row['status'] == 'completed' ? '#10B981' : '#F59E0B'; ?>; padding: 4px 10px; border-radius: 6px; font-weight: 800; font-size: 0.75rem; text-transform: uppercase;"><?php echo $row['status']; ?></span></td>
                        <td style="text-align: center;"><?php echo $row['completion_date'] ? date('d M Y', strtotime($row['completion_date'])) : '-'; ?></td>
                    </tr>
                    <?php endwhile; ?>
                </tbody>
            </table>
            ...
            <table class="employee-data-table">
                ...
                <tbody>
                    ...
                    ?>
                    <tr>
                        <td style="font-weight: 700; color: #0F172A; text-align: left;"><?php echo e($row['full_name']); ?></td>
                        <td style="text-align: left;"><?php echo e($row['exam_name']); ?></td>
                        <td style="font-weight: 800; text-align: center;"><?php echo $row['score']; ?>%</td>
                        <td style="text-align: center;">1</td>
                        <td style="text-align: center;"><span style="background: <?php echo $row['status'] == 'pass' ? '#DCFCE7' : '#FEE2E2'; ?>; color: <?php echo $row['status'] == 'pass' ? '#16A34A' : '#DC2626'; ?>; padding: 4px 10px; border-radius: 6px; font-weight: 800; font-size: 0.75rem; text-transform: uppercase;"><?php echo $row['status']; ?></span></td>
                    </tr>
                    <?php endwhile; ?>
                </tbody>
            </table>
